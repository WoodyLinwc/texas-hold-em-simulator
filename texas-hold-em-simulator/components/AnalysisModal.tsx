    ...
    return (
      ...
      >
        <div className="flex flex-col items-center gap-1">
          <span>{label}</span>
          <span className="text-xs opacity-70">{labelCn}</span>
        </div>
        {isActive && (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-gray-900 border border-gray-700 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-b border-gray-800 flex justify-between items-start bg-gray-950">
          <div>
            <h2 className="text-2xl font-bold text-white flex items-center gap-2">
              ...
              {analysis.winner === "Player"
                ? "You Won! 你赢了！"
                : analysis.winner.startsWith("Opponent")
                ? `${analysis.winner} Won ${analysis.winner.replace(
                    "Opponent",
                    "对手"
                  )} 赢了`
                : "Split Pot 平分底池"}
            ...
            <p className="text-gray-400 text-sm mt-1">
              {analysis.winningHandDescription}
            </p>
          </div>
          <button
        ...
        <div className="flex border-b border-gray-800 bg-gray-950/50">
          <TabButton stage="preflop" label="Pre-Flop" labelCn="翻牌前" />
          <TabButton stage="flop" label="Flop" labelCn="翻牌" />
          <TabButton stage="turn" label="Turn" labelCn="转牌" />
          <TabButton stage="river" label="River" labelCn="河牌" />
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            ...
            <div className="bg-gray-800/50 rounded-2xl p-6 border border-gray-700/50 flex flex-col items-center justify-center text-center gap-2 h-full">
              ...
              {isStageAvailable ? (
                ...
                <div className="text-gray-500 italic text-lg py-4">
                  Stage not reached
                  <br />
                  未达到此阶段
                </div>
            ...
            <div className="flex flex-col items-center justify-center gap-4 min-h-[160px]">
              <h3 className="text-gray-400 uppercase text-xs font-semibold tracking-wider text-center">
                Board at this Stage
                <br />
                此阶段牌面
              </h3>
              <div className="flex gap-2 justify-center">
                {visibleBoard.length > 0 ? (
                  ...
                  <div className="h-24 sm:h-28 flex items-center justify-center text-gray-600 border-2 border-dashed border-gray-800 rounded-lg w-full px-8 text-center">
          ...
          <div className="bg-gray-800/30 rounded-xl p-6 border border-gray-700/30">
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex gap-2 shrink-0">
                  {playerHand.map((c, i) => (
                    <Card
                      key={i}
                      card={c}
                      className="scale-75 origin-left w-12 h-16 sm:w-16 sm:h-24"
                    />
                  ))}
                </div>
                <div>
                  ...
                  <div className="text-sm text-gray-400">
                    {analysis.playerHandDescription}
                  </div>
                </div>
                {analysis.winner === "Player" && (
                  <Trophy className="ml-auto text-yellow-400 w-6 h-6" />
                )}
              </div>
              ...
              {opponentHands.map((hand, idx) => {
                ...
                const isWinner =
                  analysis.winner === oppName ||
                  (analysis.winner === "Split" &&
                    analysis.winningHandDescription ===
                      analysis.opponentHandDescriptions[idx]);
                return (
                  <div
                    key={idx}
                    className={`flex items-center gap-4 p-3 rounded-lg ${
                      isWinner ? "bg-red-900/30 border border-red-800" : ""
                    }`}
                  >
                    <div className="flex gap-2 shrink-0">
                      {hand.map((c, i) => (
        ...
        <div className="p-4 border-t border-gray-800 bg-gray-950 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          >
            <Home className="w-4 h-4" /> Back to Menu 返回菜单
          </button>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            >
              Play Next Hand 下一局 <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      </div>
