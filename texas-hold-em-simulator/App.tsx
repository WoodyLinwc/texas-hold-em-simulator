...
import { Loader2, User, ShieldAlert, Users, Sparkles, Smartphone, LogOut } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const checkOrientation = () => {
      ...
      // Tablets usually have a width (which becomes height in landscape) of at least 600-768px.
      // Phones usually max out around 450px width (height in landscape).
      // We set cutoff at 600px to cover large phones but exclude most tablets.
  ...
  const startNewGame = useCallback((forceStrongPlayer: boolean = false) => {
    // Reset state
    ...
    if (forceStrongPlayer) {
        // Create full deck first to search for cards
        const fullDeck = createDeck();
        
        // Select a random high rank for the pair
        const highRanks = [Rank.Ten, Rank.Jack, Rank.Queen, Rank.King, Rank.Ace];
        const randomRank = highRanks[Math.floor(Math.random() * highRanks.length)];
        
        // Find first two cards of this rank
        const cardsOfRank = fullDeck.filter(c => c.rank === randomRank);
        pHand = cardsOfRank.slice(0, 2);
        
        // Filter these out of the deck and shuffle the rest
        const idsToRemove = new Set(pHand.map(c => c.id));
        currentDeck = shuffleDeck(fullDeck.filter(c => !idsToRemove.has(c.id)));
    } else {
        // Standard shuffle and deal
    ...
    // Deal Cards to N Opponents
    const newOpponentHands: CardType[][] = [];
    
    // We iterate to create hands. We use '[]' for playerHand in generateOpponentHand 
    // because we have already physically removed cards from currentDeck, so there is no collision risk 
    // via ID check if we just pass the modified deck. 
    // However, generateOpponentHand uses `removeCardsFromDeck` internally which filters by ID.
    // So passing the current modified deck is correct.
  ...
  const handleAction = (action: 'bet' | 'check' | 'fold') => {
    if (action === 'fold') {
      ...
      // Give a small delay before showing analysis to let user register the fold
  ...
  const startAnalysis = async () => {
    ...
    // Fill the board if the game ended early (Fold or incomplete)
    // This allows the user to see "what would have happened"
    ...
    // Update board state so UI shows the full runout
  ...
  // Helper to calculate CSS classes for N opponents to avoid overlap
  const getOpponentClasses = (index: number, total: number): string => {
      const base = "absolute flex flex-col items-center gap-2 transition-all duration-500 z-10";
      
      // OPTIMIZED FOR MOBILE:
      // - Smaller scales (0.55/0.6)
      // - Higher vertical positions (top-[5%] to top-[20%]) to clear the center board area
      ...
      if (total === 2) {
          // Spread top left/right
          if (index === 0) return `${base} top-[5%] left-[20%] -translate-x-1/2 scale-[0.55] sm:scale-100 sm:top-[8%] sm:left-[25%]`;
          if (index === 1) return `${base} top-[5%] left-[80%] -translate-x-1/2 scale-[0.55] sm:scale-100 sm:top-[8%] sm:left-[75%]`;
      }
      
      if (total === 3) {
          // Mobile: Arc arrangement on top half
          // Left Side - moved up
          if (index === 0) return `${base} top-[18%] left-1 -translate-y-1/2 origin-left scale-[0.55] sm:scale-100 sm:top-[35%] sm:left-[10%]`; 
          // Top Center
          if (index === 1) return `${base} top-[2%] left-1/2 -translate-x-1/2 origin-top scale-[0.55] sm:scale-100 sm:top-[8%]`;  
          // Right Side - moved up
          if (index === 2) return `${base} top-[18%] right-1 -translate-y-1/2 origin-right scale-[0.55] sm:scale-100 sm:top-[35%] sm:right-[10%]`; 
      }
      
      if (total === 4) {
          // Mobile: 4 corners of top half
          // Lower Left - moved up
          if (index === 0) return `${base} top-[22%] left-1 -translate-y-1/2 origin-left scale-[0.55] sm:scale-100 sm:top-[35%] sm:left-[8%]`; 
          // Top Left
          if (index === 1) return `${base} top-[2%] left-[15%] -translate-x-1/2 origin-top scale-[0.55] sm:scale-100 sm:top-[8%] sm:left-[30%]`;  
          // Top Right
          if (index === 2) return `${base} top-[2%] left-[85%] -translate-x-1/2 origin-top scale-[0.55] sm:scale-100 sm:top-[8%] sm:left-[70%]`;  
          // Lower Right - moved up
          if (index === 3) return `${base} top-[22%] right-1 -translate-y-1/2 origin-right scale-[0.55] sm:scale-100 sm:top-[35%] sm:right-[8%]`; 
      }
      return base;
  };
    ...
    <div className="min-h-screen felt-bg flex flex-col font-sans overflow-hidden">
      ...
      <div className="h-14 sm:h-16 bg-gray-900/80 backdrop-blur border-b border-gray-700 flex items-center justify-between px-4 sm:px-6 z-30 relative">
        <h1 className="text-lg sm:text-xl font-bold text-emerald-400 flex items-center gap-2">
          <ShieldAlert className="w-5 h-5 sm:w-6 sm:h-6" /> <span className="hidden sm:inline">Texas Hold'em</span><span className="sm:hidden">Texas Hold 'em Simulator</span>
        </h1>
        {stage !== GameStage.Idle && (
            ...
            >
                <LogOut className="w-4 h-4" /> <span className="hidden sm:inline">Quit Game</span>
            </button>
        )}
      </div>
        ...
        {stage === GameStage.Idle ? (
           <div className="mt-10 sm:mt-0 flex flex-col items-center justify-center h-full text-center space-y-6 sm:space-y-8 animate-in zoom-in duration-300 bg-gray-900/40 p-6 sm:p-10 rounded-3xl border border-gray-700 backdrop-blur-sm mx-4">
             <div>
                ...
                <h2 className="text-2xl sm:text-4xl font-bold text-white drop-shadow-lg">Ready to Play?</h2>
                <p className="text-emerald-200 text-sm sm:text-base max-w-md mx-auto mt-2 sm:mt-4">
                Challenge the computer. The opponents play strong hands 50% of the time. 
                </p>
             </div>

             {/* Opponent Selector */}
             <div className="flex flex-col items-center gap-3">
                 <label className="text-gray-300 text-xs sm:text-sm uppercase tracking-wider font-semibold">Number of Opponents</label>
             ...
             <div className="flex flex-col gap-3 sm:gap-4 w-full max-w-xs mx-auto">
                 ...
                 >
                   Deal Cards <ArrowRightIcon className="w-5 h-5" />
                 </button>
          ...
          <>
                ...
                return (
                    ...
                    >
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-red-200 font-bold text-xs sm:text-sm bg-black/30 px-2 py-0.5 rounded-full">Opp {idx + 1}</span>
                        </div>
                        <div className="flex gap-1 sm:gap-2">
                            {hand.map((card, cIdx) => (
                                <Card 
                                    key={`opp-${idx}-${cIdx}`} 
                                    card={card} 
                                    hidden={stage !== GameStage.Showdown && stage !== GameStage.Analyzing && stage !== GameStage.Results} 
                                />
                            ))}
                        </div>
                    </div>
            ...
            {/* Community Cards (Center) - Pushed down on mobile to clear opponents */}
            <div className="absolute top-[40%] sm:top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-1 sm:gap-4 z-0">
              {board.map((card, idx) => (
                <div key={card.id} className="animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-backwards" style={{ animationDelay: `${idx * 150}ms` }}>
                  {/* Smaller cards on mobile via className override */}
                  <Card card={card} small className="w-[3.2rem] h-[4.8rem] sm:w-20 sm:h-28" />
                </div>
              ))}
              {/* Placeholders for board slots */}
              {Array.from({ length: 5 - board.length }).map((_, idx) => (
                <div key={`placeholder-${idx}`} className="w-[3.2rem] h-[4.8rem] sm:w-20 sm:h-28 rounded-lg border-2 border-dashed border-white/10 bg-white/5" />
              ))}
            </div>

            {/* Player Area (Bottom) - Adjusted bottom spacing */}
            <div className="absolute bottom-20 sm:bottom-28 flex flex-col items-center gap-2 transition-all duration-500 z-10 scale-[0.8] sm:scale-100 origin-bottom">
               <div className="flex gap-2 sm:gap-4">
                {playerHand.map((card, idx) => (
                  <Card key={`player-${idx}`} card={card} large className="shadow-2xl" />
                ))}
              </div>
              <span className="text-emerald-200 font-bold text-sm bg-black/30 px-3 py-1 rounded-full mt-2">You</span>
            </div>
          </>
        )}
      </div>
      ...
      {stage !== GameStage.Idle && stage !== GameStage.Analyzing && stage !== GameStage.Results && stage !== GameStage.Showdown && (
      ...
      {stage === GameStage.Analyzing && (
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
           <div className="bg-gray-900 p-6 sm:p-8 rounded-2xl border border-emerald-500/30 flex flex-col items-center gap-4 animate-pulse">
              ...
              <p className="text-emerald-400 font-mono text-sm sm:text-base">Calculating Odds...</p>
           </div>
        </div>
      )}
      ...
      {error && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 bg-red-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg shadow-xl z-50 flex items-center gap-2 animate-in fade-in slide-in-from-top-4 w-11/12 sm:w-auto text-sm sm:text-base">
          ...
          {error}
          <button onClick={() => setError(null)} className="ml-2 underline opacity-80 hover:opacity-100 whitespace-nowrap">Dismiss</button>
        </div>
      )}
